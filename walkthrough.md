...
You can run the automated tests in the `tests/` folder to ensure everything is working correctly:

```bash
python tests/verify_normalizer.py
python tests/verify_equalizer.py
python tests/verify_phantom.py
```

These tests use the MIDI files located in `tests_MIDIs/`.
